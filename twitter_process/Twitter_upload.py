...
def tranport_in(json_string, tran_string):
    data = json.loads(json_string)
    for item in tran_string:
        if item in data["doc"]["data"]["text"]:
            return True
    return False
...
def process_twitter_data(file_address: str, comm: object, size: int, rank: int,db,count):
    transport_string = ['bus','car', 'bike', 'tram', 'metro', 'public transport', 'lightrail', 'traffic jam', 'public transport', 'uber', 'railway', 'train', 'transit']
    f = None
    try:
        ...
        while 1:
                ...
                if current_line.startswith("{"):
                    ...
                    if "geo" in current_line:
                        if tranport_in(current_line,transport_string):
